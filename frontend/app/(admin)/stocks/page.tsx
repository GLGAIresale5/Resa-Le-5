      ...
      <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-zinc-200 px-6 py-4">
          <div>
            ...
            {isEdit && (
              <p className="mt-0.5 text-xs text-zinc-500">{CATEGORY_LABELS[item!.category]}</p>
            )}
          </div>
          <button
            type="button"
          ...
          <div className="flex items-center justify-between pt-1">
            ...
            {isEdit && onDelete && (
              confirmDelete ? (
                <div className="flex items-center gap-2">
                  <span className="text-xs font-medium text-red-600">Confirmer la suppression ?</span>
                  <button
                    type="button"
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-xs font-medium text-red-500 hover:bg-red-50"
                >
            ...
            <div className="flex gap-2">
              ...
              <button
                type="submit"
                disabled={saving}
                className="rounded-lg bg-zinc-900 px-4 py-1.5 text-xs font-semibold text-white hover:bg-zinc-700 disabled:opacity-50"
              >
...
function StockCell({
    ...
    return (
      <div className="flex items-center gap-2 rounded px-1 py-0.5">
        ...
        <input
          ...
          disabled={saving}
          className="w-14 appearance-none rounded border border-zinc-300 bg-white py-0 text-right text-sm tabular-nums text-zinc-900 focus:border-zinc-500 focus:outline-none [&::-webkit-inner-spin-button]:appearance-auto [&::-webkit-outer-spin-button]:appearance-auto"
        />
  ...
  return (
    ...
    >
      <svg
        className="h-3 w-3 text-zinc-300 opacity-0 transition group-hover:opacity-100"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        strokeWidth={2}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
      <div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-zinc-200 px-6 py-4">
          <div>
            <h2 className="text-base font-semibold text-zinc-900">Importer des Z</h2>
          ...
          {!currentScan ? (
            <div className="space-y-4">
                    ...
                    return (
                      <div key={idx} className="flex items-center justify-between px-3 py-2">
                        <div className="flex items-center gap-3">
                          <span className="text-xs font-medium text-zinc-700">{r.date}</span>
                          <span className="text-xs text-zinc-400">{linked} article(s) lié(s)</span>
                        </div>
                        <button
              ...
              <button
                ...
                disabled={scanning}
                className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-dashed border-zinc-300 py-8 text-sm font-medium text-zinc-500 transition hover:border-zinc-400 hover:text-zinc-700 disabled:opacity-50"
              >
                {scanning ? (
                  <>
            ...
            <div className="space-y-4">
              ...
              <div className="max-h-72 overflow-y-auto rounded-lg border border-zinc-100">
                <table className="w-full table-fixed">
                  ...
                  <thead>
                    <tr className="sticky top-0 border-b border-zinc-100 bg-zinc-50">
                      <th className="px-3 py-2 text-left text-xs font-medium text-zinc-400">Article (Z)</th>
                      <th className="px-3 py-2 text-left text-xs font-medium text-zinc-400">Correspondance catalogue</th>
                      <th className="px-3 py-2 text-right text-xs font-medium text-zinc-400">Qté vendue</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-zinc-50">
                    {editItems.map((item, idx) => (
                      <tr key={idx}>
                        <td className="px-3 py-2 text-sm text-zinc-700 truncate max-w-0">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-zinc-100 px-6 py-4">
          <div>
            <h2 className="text-sm font-semibold text-zinc-900">Enregistrer des sorties</h2>
            <p className="text-xs text-zinc-400">Choisissez la source de données</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 p-6">
          <button
            onClick={() => setMode("z")}
            className="flex flex-col items-center gap-3 rounded-xl border-2 border-zinc-200 p-6 text-left hover:border-zinc-400 hover:bg-zinc-50 transition"
          >
            ...
            <div>
              <p className="text-sm font-semibold text-zinc-900">Ticket Z du jour</p>
              <p className="mt-0.5 text-xs text-zinc-400">Photo du rapport journalier</p>
            </div>
          </button>
          <button
          ...
          >
            ...
            <div>
              <p className="text-sm font-semibold text-zinc-900">Historique L&apos;Addition</p>
              <p className="mt-0.5 text-xs text-zinc-400">Import CSV SalesDocumentLines</p>
            </div>
          </button>
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-zinc-100 px-6 py-4">
          <div>
            <h2 className="text-sm font-semibold text-zinc-900">Historique L&apos;Addition</h2>
            <p className="text-xs text-zinc-400">Export CSV SalesDocumentLines</p>
          </div>
          <button onClick={onClose} className="rounded-lg p-1.5 text-zinc-400 hover:bg-zinc-100 hover:text-zinc-700">
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
          ...
          {!result ? (
            <>
              <p className="text-xs text-zinc-500 leading-relaxed">
                Dans L&apos;Addition, exporte le rapport <strong className="text-zinc-700">SalesDocumentLines</strong> au format CSV.
                Sélectionne le fichier pour importer tout l&apos;historique automatiquement.
              </p>
              {error && <div className="rounded-lg bg-red-50 px-4 py-3 text-xs text-red-600">{error}</div>}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-lg bg-zinc-50 px-4 py-3 text-center">
                  <p className="text-xl font-bold text-zinc-900">{result.days}</p>
                  <p className="text-xs text-zinc-400">jours importés</p>
                </div>
                <div className="rounded-lg bg-zinc-50 px-4 py-3 text-center">
                  <p className="text-xl font-bold text-zinc-900">{result.unique_products}</p>
                  <p className="text-xs text-zinc-400">produits distincts</p>
                </div>
                <div className="rounded-lg bg-zinc-50 px-4 py-3 text-center">
                  <p className="text-xl font-bold text-zinc-900">{result.matched_to_catalogue}</p>
                  <p className="text-xs text-zinc-400">liés au catalogue</p>
                </div>
                <div className="rounded-lg bg-zinc-50 px-4 py-3 text-center">
                  <p className="text-xl font-bold text-zinc-900">{result.updated_thresholds}</p>
                  <p className="text-xs text-zinc-400">seuils recalculés</p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="flex justify-end border-t border-zinc-100 px-6 py-4">
          <button
            onClick={onClose}
            className="rounded-lg border border-zinc-200 px-4 py-1.5 text-xs font-medium text-zinc-600 hover:bg-zinc-50"
          >
              ...
              <div className="mb-4 grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={supplierName} onChange={(e) => setSupplierName(e.target.value)}
                    className="w-full rounded-lg border border-zinc-200 px-3 py-1.5 text-sm text-zinc-900 placeholder:text-zinc-400 focus:border-zinc-400 focus:outline-none"
                  ...
                  {d.items.length > 0 && (
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      {d.items.slice(0, 6).map((item) => (
                        <span key={item.id} className="rounded-full border border-zinc-100 bg-zinc-50 px-2 py-0.5 text-[11px] text-zinc-500">
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-32">
        ...
        <p className="mt-1 text-xs text-zinc-400">Initialisez le catalogue bar depuis le cadencier</p>
        <button
          onClick={handleSeed}
          disabled={seeding}
          className="mt-6 rounded-lg bg-zinc-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-zinc-700 disabled:opacity-50"
        >
              ...
              <div className="mb-3 flex items-center justify-between">
                ...
                {!bulkMode && (
                  <button
                    ...
                    className="flex items-center gap-1 rounded-lg px-2 py-1 text-xs font-medium text-zinc-400 hover:bg-zinc-100 hover:text-zinc-700 transition"
                  >
                    <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  ...
                  <tbody className="divide-y divide-zinc-50">
                    {catItems.map((item) => {
                      ...
                      return (
                        ...
                        >
                          <td className="pl-4 py-2.5 text-sm font-medium text-zinc-900 truncate">
                            {item.name}
                          </td>
                          <td className="pl-2 py-2.5 text-sm text-zinc-500 truncate">
                            {item.brand ?? "—"}
                          </td>
                          ...
                          <td className="px-4 py-2.5 text-right text-sm text-zinc-400 whitespace-nowrap">
                            <span>{item.stock_min} {item.unit}</span>
                            {(item as StockItem & { auto_thresholds?: boolean }).auto_thresholds && (
                              <span className="ml-1.5 rounded bg-violet-100 px-1 py-0.5 text-[10px] font-medium text-violet-600">auto</span>
                            )}
                          </td>
                          ...
                          <td className="px-2 py-2.5">
                            {!bulkMode && (
                              <button
                                ...
                                className="rounded p-1 text-zinc-300 hover:bg-zinc-100 hover:text-zinc-600 transition"
                                title="Modifier"
                              >
                                <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                  <path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536M9 13l6.586-6.586a2 2 0 112.828 2.828L11.828 15.828a4 4 0 01-2.828 1.172H7v-2a4 4 0 011.172-2.828z" />
...
function LivraisonsTab({
  ...
  return (
    <div className="p-8">
      <div className="mb-6 flex items-center justify-between">
        <p className="text-sm text-zinc-500">{deliveries.length} livraison(s) enregistrée(s)</p>
        <div>
          <input
            ref={fileRef}
            type="file"
              ...
              {d.items.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {d.items.slice(0, 6).map((item) => (
                    <span
                      key={item.id}
                      className="rounded-full border border-zinc-100 bg-zinc-50 px-2 py-0.5 text-[11px] text-zinc-500"
                    >
                      {item.item_name} × {item.quantity}
                    </span>
                  ))}
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="w-16 rounded-lg border border-zinc-200 px-2 py-1 text-xs text-zinc-900 placeholder:text-zinc-400 focus:border-zinc-400 focus:outline-none"
            />
          </div>
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="w-36 rounded-lg border border-zinc-200 px-2 py-1 text-xs text-zinc-900 placeholder:text-zinc-400 focus:border-zinc-400 focus:outline-none"
            />
          </div>
          <div className="flex flex-1 items-center gap-2">
            ...
            <input
              ...
              className="flex-1 rounded-lg border border-zinc-200 px-2 py-1 text-xs text-zinc-900 placeholder:text-zinc-400 focus:border-zinc-400 focus:outline-none"
            />
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6">
        {messages.length === 0 && (
          <div className="flex h-full flex-col items-center justify-center text-center">
            ...
            <p className="mt-3 text-sm font-medium text-zinc-700">Agent Stocks — GLG AI</p>
      ...
      <div className="border-t border-zinc-200 bg-white px-4 md:px-8 py-4">
        <div className="flex items-end gap-3">
          <textarea
            ...
            className="flex-1 resize-none rounded-xl border border-zinc-200 px-4 py-2.5 text-sm text-zinc-900 placeholder:text-zinc-400 focus:border-zinc-400 focus:outline-none"
          />
          <button
            onClick={sendMessage}
            disabled={loading || !input.trim()}
            className="rounded-xl bg-zinc-900 p-2.5 text-white transition hover:bg-zinc-700 disabled:opacity-40"
          >
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
const STOCK_MODE_CONFIG: Record<StockMode, { label: string; dot: string; active: string }> = {
  flux_tendu: { label: "Flux tendu", dot: "bg-amber-400", active: "border-amber-200 bg-amber-50 text-amber-700" },
  equilibre:  { label: "Équilibre",  dot: "bg-zinc-400",  active: "border-zinc-300 bg-zinc-100 text-zinc-700" },
  stock:      { label: "Stock",      dot: "bg-emerald-500", active: "border-emerald-200 bg-emerald-50 text-emerald-700" },
};
    ...
    <div className="flex flex-col">
      <header className="border-b border-zinc-200 bg-white px-4 md:px-8 py-5">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {/* Mode seuils */}
            <div className="flex items-center divide-x divide-zinc-200 overflow-hidden rounded-lg border border-zinc-200">
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium transition ${
                      isActive ? cfg.active : "bg-white text-zinc-500 hover:bg-zinc-50"
                    }`}
                  >
                    <span className={`inline-block h-2 w-2 rounded-full ${isActive ? cfg.dot : "bg-zinc-200"}`} />
                    {cfg.label}
                  </button>
                );
              })}
      ...
      {error && (
        <div className="mx-8 mt-4 rounded-lg bg-red-50 px-4 py-3 text-sm text-red-600">{error}</div>
      )}
