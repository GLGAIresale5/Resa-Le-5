...
type NavItemDef =
  | { label: string; path: string; icon: React.ReactNode; soon?: boolean; children?: never }
  | { label: string; icon: React.ReactNode; children: { label: string; path: string }[]; path?: never; soon?: never };
...
const navItemDefs: NavItemDef[] = [
  ...
  },
  {
    ...
    children: [
      { label: "Bar", path: "/stocks" },
      { label: "Cuisine", path: "/stocks-cuisine" },
    ],
  },
];
...
export default function Sidebar() {
  ...
  // Prefix all paths with the restaurant slug
  const navItems = navItemDefs.map((item) => {
    if (item.children) {
      return { ...item, children: item.children.map((c) => ({ ...c, href: `/${slug}${c.path}` })) };
    }
