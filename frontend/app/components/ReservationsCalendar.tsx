  ...
  return (
    <div className="mx-auto w-full max-w-md px-3 md:px-6 py-4 pb-10">
        ...
        {cells.map((date, i) => {
          ...
          return (
            <button
              ...
              className={`aspect-square rounded-md flex flex-col items-center justify-center gap-0.5 transition-colors border bg-neutral-900/60 border-neutral-800 hover:border-neutral-600 ${
                isToday ? "ring-1 ring-white/60" : ""
              }`}
            >
              <span className={`text-sm leading-none ${n > 0 ? "text-white font-medium" : "text-neutral-400"}`}>{dayNum}</span>
              {n > 0 ? (
                <span className="min-w-[18px] px-1 rounded-full bg-emerald-500/15 text-emerald-300 text-[10px] font-medium leading-4">{n}</span>
              ) : (
                <span className="h-4" />
              )}
            </button>
      ...
      {selectedDay && (
        ...
        >
          <div
            className="w-full max-w-sm max-h-[75vh] overflow-y-auto overscroll-contain bg-neutral-900 border border-neutral-800 rounded-2xl flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between gap-2 p-4 pb-3 border-b border-neutral-800 sticky top-0 bg-neutral-900 rounded-t-2xl">
              ...
              <button onClick={() => setSelectedDay(null)} aria-label="Fermer" className="shrink-0 h-8 w-8 flex items-center justify-center rounded text-neutral-500 hover:text-white text-sm">✕</button>
            </div>
              ...
              <div className="divide-y divide-neutral-800/60">
                {dayRes.map((r) => (
                  <div
                    ...
                    className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-neutral-800/50 active:bg-neutral-800/70 transition-colors"
                  >
                    <div className="w-12 shrink-0 text-sm font-medium text-neutral-400">{r.time?.slice(0, 5)}</div>
            ...
            <div className="p-4 pt-3 border-t border-neutral-800 sticky bottom-0 bg-neutral-900 rounded-b-2xl">
              <button
                onClick={() => { const d = selectedDay; setSelectedDay(null); onAdd(d); }}
                className="w-full py-2.5 rounded-lg text-sm font-medium bg-white text-neutral-950 hover:bg-neutral-200 transition-colors"
              >
